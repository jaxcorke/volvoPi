...
def mpl_to_ctkimage(fig):
    buf = io.BytesIO()
    fig.savefig(buf, format="png")
    buf.seek(0)
    image = Image.open(buf)
    ctk_image = customtkinter.CTkImage(light_image=image, dark_image=image, size=(image.width, image.height))
    return ctk_image
...
plotting_thread = threading.Thread(target=plots, daemon=True)
